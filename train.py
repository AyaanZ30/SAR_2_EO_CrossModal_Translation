...
"""
Training script for SAR-to-EO pix2pix using Pretrained Stable Diffusion U-Net Trunk.
...
from src.dataset import list_roi_ids, split_roi_ids, PrecomputedLatentDataset
from src.model import CDiffSETPretrainedUNet
...
@torch.no_grad()    
def sample(model, scheduler, z_sar, device, num_inference_steps = 50):
    """Iterative DDPM reverse sampling conditioned on a SAR latent"""
    z_sar = z_sar.to(device)
    sched = DDPMScheduler(num_train_timesteps = scheduler.config.num_train_timesteps)
    sched.set_timesteps(num_inference_steps, device=device)
...
def main(cfg_path, resume):
    ...
    # ───► UPGRADED: Pretrained Foundational Model Trunk Initialization ◄───
    model = CDiffSETPretrainedUNet(latent_channels = cfg["model"]["latent_channels"], base_channels = cfg["model"]["base_channels"])
    
    # ───► UPGRADED: Optimizer only tracks active non-frozen adapter weights ◄───
    trainable_params = [p for p in model.parameters() if p.requires_grad]
    optimizer = torch.optim.AdamW(trainable_params, lr=float(cfg["train"]["lr"]), weight_decay=1e-4)
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for z_x, z_y in train_loader:
            ...
            # combined input : Shape: (B, 8, 32, 32)
            u_net_input = torch.cat([z_x, z_y_noisy], dim = 1)
