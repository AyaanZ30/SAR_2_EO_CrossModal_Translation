...
class SAREncoder(nn.Module):
    ...
    """
    def __init__(self, latent_ch : int = 32, base_ch : int = 96):
        super().__init__()
...
class CDiffSETUNet(nn.Module):
    """
    Confidence Diffusion U-Net for SAR -> EO Latent Image Translation (32-channel EO-VAE).
    
    Inputs:
        - z_sar:      (B, 32, 32, 32) -> Clear SAR Structural Conditioning Latent
        - z_y_noisy:  (B, 32, 32, 32) -> Noisy Target EO Latent
        - timesteps:  (B,)          -> Current diffusion timesteps
        
    Outputs:
        - pred_noise: (B, 32, 32, 32) -> Predicted noise matching EO latent space
        - confidence: (B, 1, 32, 32)  -> Pixel-level spatial confidence map
    """
    def __init__(self, latent_ch : int = 32, base_ch : int = 96, time_dim: int = 256, num_res_blocks: int = 2):
        super().__init__()
        ...
        # gives (B x 32 x 32 x 32) noise preds (each pixel of 32 x 32 EO images across all 3 channels)
        self.noise_head = nn.Conv2d(base_ch, latent_ch, kernel_size = 3, padding = 1)
        
        # gives (B x 1 x 32 x 32) confidence values (4 SAR images in a batch => 1 channel per image) => pixel level conf scores
    ...
    def forward(self, z_sar, z_y_noisy, timesteps) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Ingestion & multiscale additive conditioning
        x1 = self.in_conv(z_y_noisy) + sar_f0

        x2 = self.down1(x1, t_emb) + sar_f1
        x3 = self.down2(x2, t_emb) + sar_f2
        x4 = self.down3(x3, t_emb) + sar_f3

        mid = self.bottleneck(x4)
        
        # Reconstruction flow with skip connections
        u3 = self.up3(mid, x3, t_emb)      # (B, base_ch*4, 8, 8)
        u2 = self.up2(u3, x2, t_emb)       # (B, base_ch*2, 16, 16)
        u1 = self.up1(u2, x1, t_emb)       # (B, base_ch, 32, 32)
        
        pred_noise = self.noise_head(u1)   # (B, 32, 32, 32)
        confidence = self.confidence_head(u1)  # (B, 1, 32, 32)
